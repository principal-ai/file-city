...
export class GridLayoutManager {
     ...
     */
    splitTreeIntoGrid(fileTree, config) {
        const grid = new Map();
     ...
     */
    calculateCellBounds(totalWidth, totalHeight, row, col, rows, cols, padding = 0, config) {
     ...
     */
    assignDirectoriesToCells(root, config, assignedItems) {
        ...
        for (const [_cellName, cellConfig] of sortedCells) {
            ...
            for (const { item, path } of allItemsWithPaths) {
                ...
                if (matches) {
                    ...
                    if ('children' in item) {
                        const markDescendantsAssigned = (node, parentPath) => {
     ...
     */
    collectAllItemsWithPaths(node, basePath = '') {
        const results = [];
     ...
     */
    calculateStats(node, stats, depth) {
