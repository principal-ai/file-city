 ...
 */
export function validateCodebaseViewConfig(config) {
    const issues = [];
    const suggestions = [];
    ...
    if (!config.cells || typeof config.cells !== 'object') {
        ...
        cellNames.forEach(cellName => {
            ...
            if (!Array.isArray(cell.coordinates) || cell.coordinates.length !== 2) {
                ...
                if (!Number.isInteger(row) || !Number.isInteger(col)) {
                    ...
                    if (cellsUsed.has(cellKey)) {
                        ...
                        suggestions.push(`Consider using priority values to control which cell takes precedence at position [${row},${col}]`);
                    }
            ...
            const cellAsUIType = cell; // Cast to avoid TS errors during migration
            const colorFromUI = cellAsUIType.color;
            const colorFromMetadata = cell.metadata?.ui?.color;
            ...
            if (actualColor !== undefined) {
                if (typeof actualColor !== 'string') {
                    ...
                    suggestions.push(`Use hex colors (#RRGGBB), RGB(r,g,b), or CSS color names for "${cellName}"`);
                }
            }
        });
    ...
    if (uiMetadata?.cellLabelHeightPercent !== undefined) {
        if (!Number.isFinite(uiMetadata.cellLabelHeightPercent) ||
 ...
 */
function createReport(issues, suggestions) {
 ...
 */
export function autoFixGridConfig(config) {
    const fixed = { ...config };
...
//# sourceMappingURL=GridLayoutConfigValidator.js.map